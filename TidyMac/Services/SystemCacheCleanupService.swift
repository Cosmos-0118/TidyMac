...
final class SystemCacheCleanupService: CleanupService {
    ...
    private func scanCustomTargets(_ targets: [(path: String, name: String)]) -> CleanupCategory {
        ...
        for target in targets {
            ...
            do {
                ...
                let reasons = [
                    CleanupReason(code: "target", label: "Targeted directory", detail: detail)
                ]
                items.append(
        ...
        let error =
            failures.isEmpty
            ? nil : "Limited preview access for: \(failures.joined(separator: ", "))."
    ...
    private func scanComprehensiveTargets() -> CleanupCategory {
        ...
        let homeLibrary = fileManager.homeDirectoryForCurrentUser.appendingPathComponent(
        ...
        let groupContainers = homeLibrary.appendingPathComponent(
            "Group Containers", isDirectory: true)
        let appContainers = homeLibrary.appendingPathComponent("Containers", isDirectory: true)
        let applicationSupport = homeLibrary.appendingPathComponent(
        ...
        let crashReporter = homeLibrary.appendingPathComponent(
            "Application Support/CrashReporter", isDirectory: true)
        let coreSimulatorCaches = homeLibrary.appendingPathComponent(
            "Developer/CoreSimulator/Caches", isDirectory: true)

        aggregatedItems.append(
            contentsOf: collectChildEntries(
                under: userCaches,
                includeFiles: false,
        ...
        aggregatedItems.append(
            contentsOf: collectMatchingDirectories(
        ...
        aggregatedItems.append(
            contentsOf: collectContainerCaches(
        ...
        let inventoryResult = inventoryService.discoverCandidates(sources: [
            .browserCaches, .orphanedApplicationSupport, .orphanedPreferences, .sharedInstallers,
        ])
        if !inventoryResult.candidates.isEmpty {
            let inventoryItems = inventoryResult.candidates.map {
                CleanupCategory.CleanupItem(candidate: $0)
            }
        ...
        let note =
            "Caches, logs, and temporary files are regenerated automatically by macOS and your apps."
        ...
        if failures.isEmpty {
            ...
            error =
                "Limited access to: \(limited.joined(separator: ", ")). Grant Full Disk Access for more results."
        }
        ...
        do {
            ...
            for entry in contents {
                ...
                if entryIsDirectory {
                    ...
                    let name = nameTransform?(entry) ?? displayName(for: entry)
                    let sizeValue =
                        values.totalFileAllocatedSize ?? values.fileAllocatedSize ?? values.fileSize
        ...
        do {
            ...
            for entry in contents {
                ...
                if let match = keywords.first(where: { lowercased.contains($0) }) {
                    ...
                    let reasons = [
                        CleanupReason(
                            code: "keyword", label: "Matches \"\(match)\"", detail: detail)
                    ]
                    results.append(
                        CleanupCategory.CleanupItem(
                            path: entry.path,
    ...
    private func sortDiscoveredItems(_ items: [CleanupCategory.CleanupItem]) -> [CleanupCategory
        .CleanupItem]
    {
    ...
    private func collectContainerCaches(
        ...
        var results: [CleanupCategory.CleanupItem] = []
        var isDirectory: ObjCBool = false
        guard fileManager.fileExists(atPath: containersRoot.path, isDirectory: &isDirectory),
            isDirectory.boolValue
        else {
            return results
        }
        ...
        do {
            ...
            for container in containers {
                ...
                guard fileManager.fileExists(atPath: container.path, isDirectory: &isDir),
                    isDir.boolValue
                else { continue }

                let cachePath = container.appendingPathComponent(
                    "Data/Library/Caches", isDirectory: true)
                var cacheIsDir: ObjCBool = false
                guard fileManager.fileExists(atPath: cachePath.path, isDirectory: &cacheIsDir),
                    cacheIsDir.boolValue
                else { continue }
                ...
                do {
                    ...
                    let reasons = [
                        CleanupReason(code: "container-cache", label: detail, detail: detailText)
                    ]
                    results.append(
                        CleanupCategory.CleanupItem(
                            path: cachePath.path,
    ...
    private func performCleanup(
        ...
        guard !items.isEmpty else {
            ...
            continuation.resume(
                returning: CleanupOutcome(
                    success: true, message: "No cleanup targets selected.", recoverySuggestion: nil)
            )
            return
        }
    ...
    private struct ActionableItems {
        ...
        var emptyOutcome: CleanupOutcome {
            let dryRunMessage =
                "Dry run: no cleanup items processed because selections are protected."
    ...
    private func filterActionableItems(from items: [CleanupCategory.CleanupItem]) -> FilterResult {
        do {
            ...
            if !excludedSelections.isEmpty {
                metadata["excludedSamples"] = Array(excludedSelections.prefix(3)).joined(
                    separator: ", ")
            }
    ...
    private func executeCleanup(
        ...
        let syncQueue = DispatchQueue(label: "com.tidymac.systemCleanup.sync")
        let workerQueue = DispatchQueue(
            label: "com.tidymac.systemCleanup.worker", qos: .userInitiated, attributes: .concurrent)
                        ...
                        if isSystemProtectedCachePath(path) {
                            ...
                            if shouldLog {
                                Diagnostics.warning(
                                    category: .cleanup,
                                    message:
                                        "Cache path is protected by macOS and cannot be removed automatically.",
                                    metadata: ["path": path]
                                )
                            }
                        } else {
                    ...
                    if isSystemProtectedCachePath(path) {
                        ...
                        if shouldLog {
                            Diagnostics.warning(
                                category: .cleanup,
                                message:
                                    "Cache directory is protected by macOS and cannot be inspected automatically.",
                                metadata: ["path": path]
                            )
                        }
                    } else {
                        ...
                        Diagnostics.warning(
                            category: .cleanup,
                            message:
                                "Administrator permission required to inspect cache directory.",
                            metadata: ["path": path]
                        )
                    }
                } else if isPermissionError(error) {
        ...
        if dryRun {
            ...
            if success {
                recovery =
                    skippedProtected > 0
                    ? "Adjust your exclusion list if you want to include the skipped paths." : nil
            } else {
        ...
        if failures.isEmpty {
            ...
            if !systemProtectedPaths.isEmpty {
                message +=
                    " macOS protected \(systemProtectedPaths.count) item(s); they remain for safety."
            }
            ...
            continuation.resume(
                returning: CleanupOutcome(success: true, message: message, recoverySuggestion: nil))
            return
        }
        ...
        if privilegedCancelled {
            message =
                "Administrator permission was required to remove \(failureCount) cache item(s)."
        ...
        if hasSystemProtectedFailures {
            recoveryComponents.append(
                "Some cache paths are protected by macOS System Integrity Protection and can't be removed automatically."
            )
        }
        if privilegedCancelled {
            recoveryComponents.append(
                "Re-run the cleanup and approve the administrator prompt to finish removing protected items."
            )
            ...
            recoveryComponents.append("Grant access or run TidyMac with elevated permissions.")
        }
        if skippedProtected > 0 {
        ...
        let recovery =
            recoveryComponents.isEmpty
            ? "Grant access or run TidyMac with elevated permissions."
        ...
        continuation.resume(
            returning: CleanupOutcome(
                success: false, message: message, recoverySuggestion: recovery))
    }
}
