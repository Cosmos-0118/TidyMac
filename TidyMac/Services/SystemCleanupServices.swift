    ...
    func scan() async -> CleanupCategory {
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                directoryLoop: for path in candidateDirectories {
                    ...
                    for case let fileURL as URL in enumerator {
                        ...
                        do {
                            ...
                            let age =
                                Calendar.current.dateComponents([.day], from: modified, to: Date())
                                .day ?? 0
                            if Int64(size) >= thresholdBytes && age >= maxAgeDays {
                                let detail =
                                    age > 0 ? "Last modified \(age) days ago" : "Recently modified"
                                let reasons: [CleanupReason] = [
                                    CleanupReason(
                                        code: "size", label: "Larger than threshold",
                                        detail: formatByteCount(Int64(size))),
                ...
                let note =
                    items.count >= maxResults ? "Showing the first \(maxResults) results." : nil
                continuation.resume(
                    returning: CleanupCategory(
                        step: .largeFiles, items: items, error: error, note: note))
            }
        }
    }
    ...
    func execute(
        ...
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                guard !items.isEmpty else {
                    continuation.resume(
                        returning: CleanupOutcome(
                            success: true, message: "No large files selected.",
                ...
                progressTracker.registerAdditionalUnits(
                    actionableItems.count, update: progressUpdate)
                ...
                let syncQueue = DispatchQueue(label: "com.tidymac.largeFiles.sync")
                let workerQueue = DispatchQueue(
                    label: "com.tidymac.largeFiles.worker", qos: .userInitiated,
                ...
                if dryRun {
                    ...
                    if success {
                        recovery =
                            skippedProtected > 0
                            ? "Adjust your exclusion list if you want to include the skipped paths."
                            : nil
                    } else {
                        recovery =
                            "Some files could not be inspected. Adjust your selection and retry."
                    }
                    ...
                    Diagnostics.info(
                        ...
                        metadata: [
                            "success": success ? "true" : "false", "selected": "\(removedCount)",
                        ]
                    )
                ...
                if !privilegedTargets.isEmpty {
                    ...
                    case .cancelled:
                        ...
                        Diagnostics.warning(
                            category: .cleanup,
                            message:
                                "Administrator prompt was cancelled during large file cleanup.",
                ...
                if failures.isEmpty {
                    ...
                    continuation.resume(
                        returning: CleanupOutcome(
                            success: true, message: message, recoverySuggestion: nil))
                    return
                }
                ...
                if privilegedCancelled {
                    recovery =
                        "Re-run the cleanup and approve the administrator prompt to delete the flagged files."
                } else if let privilegedFailureMessage, !privilegedFailureMessage.isEmpty {
                    recovery = privilegedFailureMessage
                } else {
                    recovery =
                        "Check permissions or close applications using the files before retrying."
                }
            ...
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                for target in targets {
                    ...
                    do {
                        ...
                        let reasons = [
                            CleanupReason(code: "xcode", label: target.name, detail: detail)
                        ]
                        items.append(
                            CleanupCategory.CleanupItem(
                                path: path,
                ...
                let error =
                    failures.isEmpty
                    ? nil : "Close Xcode to scan: \(failures.joined(separator: ", "))."
                continuation.resume(
                    returning: CleanupCategory(step: .xcodeArtifacts, items: items, error: error))
            }
        }
    }
        ...
        await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                guard !items.isEmpty else {
                    continuation.resume(
                        returning: CleanupOutcome(
                            success: true, message: "No Xcode artifacts selected.",
                            recoverySuggestion: nil))
                    return
                }
                ...
                guard !actionableItems.isEmpty else {
                    ...
                    if dryRun {
                        message =
                            "Dry run: no Xcode items processed because selections are protected."
                        recovery = "Update your exclusion list before running the cleanup."
                    } else {
                        message =
                            "Cleanup skipped. The selected Xcode paths are protected by exclusions."
                        recovery = "Adjust your exclusion list and retry."
                    }
                    continuation.resume(
                        returning: CleanupOutcome(
                ...
                let syncQueue = DispatchQueue(label: "com.tidymac.xcodeCleanup.sync")
                let workerQueue = DispatchQueue(
                    label: "com.tidymac.xcodeCleanup.worker", qos: .userInitiated,
                    attributes: .concurrent)
                let concurrencyLimit = max(2, ProcessInfo.processInfo.activeProcessorCount)
                let semaphore = DispatchSemaphore(value: concurrencyLimit)
                ...
                for item in actionableItems {
                    ...
                    do {
                        ...
                        if !contents.isEmpty {
                            progressTracker.registerAdditionalUnits(
                                contents.count, update: progressUpdate)
                        }
                ...
                if dryRun {
                    ...
                    Diagnostics.info(
                        ...
                        metadata: [
                            "success": success ? "true" : "false", "selected": "\(deletedItems)",
                        ]
                    )
                ...
                if privilegedCancelled {
                    recovery =
                        "Re-run the cleanup and approve the administrator prompt to purge protected Xcode artifacts."
                } else if let privilegedFailureMessage, !privilegedFailureMessage.isEmpty {
                    recovery = privilegedFailureMessage
                } else {
                    recovery = "Ensure Xcode is closed and TidyMac has the required permissions."
                }
