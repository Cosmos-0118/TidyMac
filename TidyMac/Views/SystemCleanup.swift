...
struct SystemCleanup: View {
	...
	init(
		...
		init(
			...
			if previewStates.isEmpty {
				previewModel.stepStates = Dictionary(
					uniqueKeysWithValues: CleanupStep.allCases.map { ($0, .pending) })
			} else {
	...
	var body: some View {
		ZStack {
			...
			.animation(
				.spring(response: 0.5, dampingFraction: 0.85, blendDuration: 0.2),
				value: showOverlay)
		}
	...
	private var header: some View {
		VStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
			VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
				...
				Text(
					"Review recommended cleanup steps before deleting caches or developer artifacts."
				)
	...
	private var scanningCard: some View {
		StatusCard(
			title: "Scanning Cleanup Targets", iconName: "magnifyingglass.circle.fill",
					...
					.overlay(alignment: .center) {
						VStack(spacing: DesignSystem.Spacing.large) {
							HStack(spacing: DesignSystem.Spacing.small) {
								Image(
									systemName: viewModel.isScanning
										? "sparkles" : "arrow.2.circlepath"
								)
							...
							VStack(spacing: DesignSystem.Spacing.small) {
								ProgressView(
									value: viewModel.isRunning ? viewModel.overallProgress : nil
								)
								...
								Text(
									viewModel.isScanning
										? "Gathering cache, large file, and Xcode artifact info"
										: progressDescription
								)
								.font(DesignSystem.Typography.caption)
								.foregroundColor(palette.secondaryText)
	...
	private var emptyStateCard: some View {
		StatusCard(
			title: "No Cleanup Targets", iconName: "checkmark.circle.fill",
			accent: palette.accentGreen
		) {
			Text(
				"TidyMac didn’t find any recommended cleanup actions. Rescan later to keep things tidy."
			)
			.font(DesignSystem.Typography.body)
			.foregroundColor(palette.primaryText)
		}
	...
	private func binding(for step: CleanupStep) -> Binding<CleanupCategory>? {
		guard let index = viewModel.categories.firstIndex(where: { $0.step == step }) else {
			return nil
		}
...
private struct CategorySidebarRow: View {
	...
	var body: some View {
		...
		.overlay(
			RoundedRectangle(cornerRadius: 12, style: .continuous)
				.stroke(
					isSelected ? palette.accentGreen.opacity(0.6) : palette.accentGray.opacity(0.2),
					lineWidth: 1)
		)
	}
	...
	private var sidebarSummary: String {
		let countSummary =
			...
			? "\(category.selectedCount)/\(category.totalCount)" : "No items"
...
private struct CategoryDetailView: View {
	...
	private var summaryLine: String {
		let countSummary =
			category.totalCount > 0
			? "\(category.selectedCount) of \(category.totalCount) selected" : "No items detected"
		if let size = category.selectedSize ?? category.totalSize {
			return "\(countSummary) • ~\(formatBytes(size))"
		}
		return countSummary
	...
	private var headerRow: some View {
		HStack(alignment: .firstTextBaseline, spacing: DesignSystem.Spacing.small) {
			...
			if let confidence = item.confidence {
				chip(
					text: "\(confidence.tier.displayName) • \(confidence.scoreDescription)",
					icon: "shield.checkerboard", color: riskColor)
			}
...
private struct WrappingTags<Data: RandomAccessCollection, Content: View>: View
...
extension CleanupReason {
	fileprivate var labelLine: String {
...
private func guardDescriptor(for decision: DeletionGuard.Decision) -> (
	title: String, icon: String, color: (DesignSystemPalette) -> Color
) {
	switch decision {
	case .allow:
...
private func formatBytes(_ bytes: Int64) -> String {
	...
	private enum SystemCleanupPreviewData {
		...
		static let states: [CleanupStep: CleanupStepState] = [
			...
			.largeFiles: .pending,
			.xcodeArtifacts: .failure(
				message: "Failed to delete DerivedData", recovery: "Close Xcode and try again."),
		]
	...
	#Preview("System Cleanup • Overview") {
		PreviewShowcase {
			...
			.environment(\.designSystemPalette, .tidyMacDark)
		}
	}
