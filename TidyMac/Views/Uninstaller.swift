...
struct Uninstaller: View {
    ...
    init(previewApplications: [Application]) {
        ...
        _viewModel = StateObject(
            wrappedValue: UninstallerViewModel(applications: previewApplications))
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
                ...
                Text(
                    "Review installed apps, understand their install location, and uninstall with confidence."
                )
                    ...
                    LazyVStack(alignment: .leading, spacing: DesignSystem.Spacing.medium) {
                        ForEach(viewModel.filteredGroups()) { group in
                            VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
                                ...
                                LazyVStack(
                                    alignment: .leading, spacing: DesignSystem.Spacing.xSmall
                                ) {
        ...
        if let app = viewModel.selectedApplication {
            ScrollView {
                VStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
                    ...
                    if app.requiresRoot {
                        StatusCard(
                            title: "Admin Privileges Required", iconName: "lock.shield.fill",
                            accent: palette.accentRed
                        ) {
                            Text(
                                "Apps installed to \(app.installLocation.displayName) require administrator approval to remove. Run TidyMac with elevated privileges or uninstall manually if the automatic removal fails."
                            )
                            .font(DesignSystem.Typography.body)
                            .foregroundColor(palette.primaryText)
                            FullDiskAccessButton()
                    ...
                    HStack(spacing: DesignSystem.Spacing.medium) {
                        Button {
                            ...
                            Label(
                                app.requiresRoot ? "Request Uninstall" : "Uninstall",
                                systemImage: "trash")
                        }
            ...
            StatusCard(
                title: "Loading Applications", iconName: "arrow.triangle.2.circlepath",
            ...
            StatusCard(
                title: "No Applications Found", iconName: "macwindow", accent: palette.accentGray
            ) {
                Text(
                    "We couldn't find any apps to uninstall. Refresh after installing or relocating apps."
                )
                .font(DesignSystem.Typography.body)
                .foregroundColor(palette.primaryText)
            }
            ...
            StatusCard(
                title: "Select an Application", iconName: "cursorarrow.rays",
    ...
    private func sidebarRow(for app: Application) -> some View {
        ...
        return Button {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(
                        isSelected ? palette.accentGreen.opacity(0.4) : Color.clear, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
                    ...
                    VStack(spacing: DesignSystem.Spacing.large) {
                        ...
                        VStack(spacing: DesignSystem.Spacing.small) {
                            ProgressView()
                                ...
                                .tint(palette.accentGreen)
                            Text(
                                "Scanning installed apps and related data. Feel free to continue browsing."
                            )
                            .font(DesignSystem.Typography.caption)
                            .foregroundColor(palette.secondaryText)
                            .multilineTextAlignment(.center)
    ...
    private func uninstallReviewSheet(for app: Application) -> some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.xSmall) {
                ...
                Text(
                    "TidyMac found these items linked to \(app.name). Review them before continuing the uninstall request."
                )
                .font(DesignSystem.Typography.body)
                .foregroundColor(palette.secondaryText)
            }
            ...
            if reviewIsLoading {
                ...
                Text(
                    "No supporting files were detected. Only the application bundle will be removed."
                )
                .font(DesignSystem.Typography.body)
                .foregroundColor(palette.secondaryText)
                .frame(maxWidth: .infinity, alignment: .leading)
