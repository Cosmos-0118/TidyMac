...
struct Dashboard: View {
        ...
        ScrollView {
            VStack(spacing: DesignSystem.Spacing.large) {
                ...
                LazyVGrid(columns: columns, spacing: DesignSystem.Spacing.large) {
                    StatusCard(
                        title: "Storage", iconName: "internaldrive", accent: palette.accentGreen
                    ) {
                        storageSection
                    }

                    StatusCard(
                        title: "Battery", iconName: "bolt.horizontal", accent: palette.accentGreen
                    ) {
                        batterySection
                    }
                    ...
                    StatusCard(title: "CPU", iconName: "cpu", accent: palette.accentRed) {
                        gaugeTile(
                            title: "CPU Usage", value: viewModel.cpuUsage, icon: "cpu",
                    ...
                    {
                        gaugeTile(
                            title: "Memory Usage", value: viewModel.memoryUsage, icon: "memorychip",
                    ...
                    {
                        gaugeTile(
                            title: "Disk Usage", value: viewModel.diskUsage, icon: "externaldrive",
                            accent: palette.accentGray)
                    }

                    StatusCard(
                        title: "Uptime", iconName: "clock.arrow.2.circlepath",
                        accent: palette.accentGray
                    ) {
                        uptimeSection
                    }
        ...
        StatusCard(title: "System Snapshot", iconName: "sparkles", accent: palette.accentGreen) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.large) {
                HStack(spacing: DesignSystem.Spacing.large) {
                    compactMetric(
                        title: "CPU", value: viewModel.cpuUsage, accent: palette.accentRed)
                    compactMetric(
                        title: "Memory", value: viewModel.memoryUsage, accent: palette.accentGreen)
                    compactMetric(
                        title: "Disk", value: viewModel.diskUsage, accent: palette.accentGray)
                }
        ...
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
            if let battery = viewModel.batteryInfo {
                ...
                if let timeRemaining = battery.timeRemainingMinutes,
                    battery.powerSourceState == kIOPSBatteryPowerValue
                {
    ...
    private func batteryStatusText(_ battery: BatteryInfo) -> String {
        ...
        return battery.isCharging
            ? "Charging" : (battery.isCharged ? "Fully charged" : "On battery")
    }
...
struct DashboardMetricSnapshot {
    ...
    private enum DashboardPreviewData {
        static var storage: StorageInfo? = {
            guard let asset = NSDataAsset(name: "DashboardSample") else {
                return StorageInfo(
                    totalSpace: 512_000_000_000, freeSpace: 210_000_000_000,
                    usedSpace: 302_000_000_000)
            }
    ...
    #Preview("Dashboard • Loaded") {
        Dashboard(
            previewStorageInfo: DashboardPreviewData.storage,
            previewMetrics: DashboardPreviewData.metrics,
            previewBattery: DashboardPreviewData.battery, previewUptime: 98_000
        )
    ...
    #Preview("Dashboard • Error") {
        Dashboard(
            previewStorageInfo: nil,
            previewMetrics: DashboardMetricSnapshot(cpu: 37, memory: 55, disk: 81),
            previewBattery: nil, previewUptime: 2_000
        )
        .environment(\.designSystemPalette, .tidyMacDark)
    }
#endif
