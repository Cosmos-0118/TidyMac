...
struct DesignSystemPalette: Equatable {
    ...
    static let tidyMacDark = DesignSystemPalette(
...
private struct DesignSystemPaletteKey: EnvironmentKey {
    static let defaultValue: DesignSystemPalette = .tidyMacDark
}
...
struct PrimaryActionButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .shadow(
                color: palette.accentGreen.opacity(configuration.isPressed ? 0.15 : 0.35),
                radius: configuration.isPressed ? 3 : 8, y: configuration.isPressed ? 1 : 4
            )
...
struct DestructiveButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
            .shadow(
                color: palette.accentRed.opacity(configuration.isPressed ? 0.15 : 0.35),
                radius: configuration.isPressed ? 3 : 8, y: configuration.isPressed ? 1 : 4
            )
            .animation(.easeOut(duration: 0.15), value: configuration.isPressed)
    }
