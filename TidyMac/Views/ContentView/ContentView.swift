...
struct ContentView: View {
    private let palette = DesignSystemPalette.tidyMacDark
...
struct Sidebar: View {
    ...
    private var searchField: some View {
        ...
        .accessibilityLabel("Search TidyMac sections")
    }
    ...
    private func destinationRow(for item: SidebarItem) -> some View {
        ...
        return Button {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(
                        isSelected ? accent.opacity(0.45) : palette.accentGray.opacity(0.15),
                        lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
