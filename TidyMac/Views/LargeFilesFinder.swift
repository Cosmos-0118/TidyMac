...
struct LargeFilesFinder: View {
    ...
    var body: some View {
        ZStack {
            ...
            .animation(
                .spring(response: 0.45, dampingFraction: 0.85, blendDuration: 0.18),
                value: showOverlay)
        }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
            ...
            HStack(spacing: DesignSystem.Spacing.medium) {
                Text(
                    viewModel.scanCompleted ? scanSummary : "Press Scan to map large and old files."
                )
                ...
                Button {
                    ...
                    Label(
                        viewModel.scanCompleted ? "Rescan" : "Scan",
                        systemImage: viewModel.scanCompleted ? "arrow.clockwise" : "play.circle")
                }
    ...
    private var contentSection: some View {
        ...
        if viewModel.isScanning {
            ...
            if viewModel.largeFiles.isEmpty {
                StatusCard(
                    title: "No Findings", iconName: "checkmark.circle", accent: palette.accentGreen
                ) {
                    Text(
                        "Great news! We didn't detect any large, outdated files that need attention."
                    )
    ...
    private var readyState: some View {
        StatusCard(
            title: "Ready to Scan", iconName: "folder.badge.clock", accent: palette.accentGreen
        ) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
    ...
    private var permissionAlert: some View {
        if viewModel.permissionIssue, let message = viewModel.permissionMessage {
            StatusCard(
                title: "Permissions Required", iconName: "lock.shield", accent: palette.accentRed
            ) {
                Text(message)
                    .font(DesignSystem.Typography.body)
                    .foregroundColor(palette.primaryText)
        ...
        VStack(spacing: DesignSystem.Spacing.medium) {
            Table(viewModel.largeFiles, selection: selectionBinding, sortOrder: $sortOrder) {
                ...
                TableColumn("Actions") { file in
                    ...
                    .help(
                        viewModel.isExcluded(file.id)
                            ? "File excluded from removal." : "Delete \(file.name) now")
                }
                .width(140)
            }
    ...
    private var footerBar: some View {
        HStack(spacing: DesignSystem.Spacing.medium) {
            let deletableSelection = viewModel.largeFiles.filter {
                viewModel.selection.contains($0.id) && !viewModel.isExcluded($0.id)
            }
            ...
            Text(
                "\(viewModel.largeFiles.count) flagged • \(viewModel.excludedFileIDs.count) excluded"
            )
            .font(DesignSystem.Typography.caption)
            .foregroundColor(palette.secondaryText)
        }
                ...
                .overlay(alignment: .center) {
                    VStack(spacing: DesignSystem.Spacing.large) {
                        ...
                        VStack(spacing: DesignSystem.Spacing.small) {
                            ProgressView(
                                value: viewModel.totalFiles > 0
                                    ? Double(viewModel.scannedFiles) : nil,
                            ...
                            Text(
                                "We’re mapping your disks to find large and old files. You can keep browsing."
                            )
                            .font(DesignSystem.Typography.caption)
                            .foregroundColor(palette.secondaryText)
                            ...
                            if viewModel.totalFiles > 0 {
                                Text(
                                    "Scanned \(viewModel.scannedFiles) of \(viewModel.totalFiles) files"
                                )
                                .font(DesignSystem.Typography.caption)
                                .foregroundColor(palette.secondaryText)
                            }
    ...
    private var loadingState: some View {
        StatusCard(
            title: "Scanning", iconName: "magnifyingglass.circle.fill", accent: palette.accentGreen
        ) {
            VStack(alignment: .leading, spacing: DesignSystem.Spacing.small) {
                Text("Scanning for large and old files…")
                    ...
                    .foregroundColor(palette.primaryText)
                ProgressView(
                    value: Double(viewModel.scannedFiles),
                    total: Double(max(viewModel.totalFiles, 1))
                )
                .tint(palette.accentGreen)
    ...
    struct LargeFilesPreviewState {
        ...
        #Preview("Large Files • Loading") {
            LargeFilesFinder(
                previewState: LargeFilesPreviewState(
                    files: [],
                    scanCompleted: false,
                    totalFiles: 2800,
                    scannedFiles: 1260,
        ...
        #Preview("Large Files • Loaded") {
            LargeFilesFinder(
                previewState: LargeFilesPreviewState(
                    ...
                    sortOrder: [KeyPathComparator(\.size, order: .reverse)],
                    excludedFileIDs: Set(
                        [LargeFilesPreviewData.loaded.first?.id].compactMap { $0 }),
                    selection: [],
