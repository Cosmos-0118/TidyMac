...
final class UninstallerViewModel: ObservableObject {
    ...
    func uninstall(_ app: Application) async {
        do {
            ...
            banner = UninstallBanner(
                ...
                message:
                    "We couldn't remove \(app.name). Try again or remove it manually from Finder.",
                success: false,
                requiresFullDiskAccess: false
            )
        }
    ...
    var appSummary: String {
        ...
        if searchText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            return total == 1
                ? "1 installed application." : "\(total) installed applications detected."
        }
    ...
    private func handleUninstallError(_ error: ApplicationUninstallError, for app: Application) {
        ...
        case .permissionDenied:
            banner = UninstallBanner(
                title: "Uninstall Failed",
                message:
                    "TidyMac needs additional permissions to remove \(app.name). Grant Full Disk Access or run with administrator privileges.",
                success: false,
