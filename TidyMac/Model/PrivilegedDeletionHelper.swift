...
import Foundation
        ...
        private static func requestUserConfirmation(for paths: [String]) -> Bool {
                ...
                func presentAlert() -> Bool {
                    ...
                    alert.informativeText =
                        "TidyMac needs administrator privileges to remove \(itemCount) protected item\(itemCount == 1 ? "" : "s").\n\nAffected path: \(message)"
    ...
    private final class PrivilegedBlessedRemover {
        ...
        private let helperIdentifier = "com.tidymac.cleanuphelper"
        ...
        func remove(paths: [String]) -> PrivilegedDeletionHelper.Result {
            guard helperExecutableExists() else {
                #if DEBUG
                    NSLog(
                        "Privileged helper %@ is not bundled with the app – falling back to AppleScript escalation.",
                        helperIdentifier)
                #endif
            ...
            case .success(let authorization):
                ...
                guard
                    let connection = PrivilegedHelperConnection(
                ...
                connection.proxy.removeItems(at: paths) { success, message in
                    if response == nil {
                        response =
                            ...
                            : .failure(
                                message: message
                                    ?? "Privileged helper failed to delete the selected items.")
                        semaphore.signal()
                    }
                }
                ...
                if semaphore.wait(timeout: .now() + 30) == .timedOut {
                    response = .failure(
                        message: "Timed out waiting for privileged helper response.")
                }
        ...
        private func blessHelperIfNeeded(with authorization: AuthorizationRef) -> Bool {
            ...
            let blessed = SMJobBless(
                kSMDomainSystemLaunchd, helperIdentifier as CFString, authorization, &cfError)
    ...
    private final class PrivilegedHelperConnection {
        ...
        init?(helperIdentifier: String, errorHandler: @escaping (String) -> Void) {
            ...
            connection.remoteObjectInterface = NSXPCInterface(
                with: PrivilegedDeletionXPCProtocol.self)
            ...
            guard
                ...
                }) as? PrivilegedDeletionXPCProtocol
            else {
                connection.invalidate()
                return nil
            }
    ...
    private final class AppleScriptFallbackRemover {
        func remove(paths: [String]) -> PrivilegedDeletionHelper.Result {
            ...
            do {
                ...
                return .failure(
                    message:
                        "Unable to request administrator privileges: \(error.localizedDescription)")
            }
    ...
    enum PrivilegedDeletionHelper {
        ...
        static func remove(paths: [String]) -> Result {
            .failure(message: "Elevated deletion unavailable on this platform.")
        }
    }
